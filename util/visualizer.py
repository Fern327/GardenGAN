        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                if label_html_row != '':
                    label_html += '<tr>%s</tr>' % label_html_row
                try:
                    # real_A（3*512*512）,fake_B（3*512*1024）,real_B（3*512*1024）
                    # 三张图片大小不同，显示有问题，real_A单独显示一个窗口
                    image_numpy = util.tensor2im(images[0])
                    self.vis.image(image_numpy, opts=dict(title='Real_A'),
                                   win=self.display_id + 1)
                    # fake_B与real_B另显示在窗口中
                    self.vis.images(images[1:], nrow=ncols-1, win=self.display_id + 2,
